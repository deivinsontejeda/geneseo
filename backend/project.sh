...
print_error  (){ printf "$(tput setaf 1)[composer] %b$(tput sgr0)\n" "$1" >&2; }
print_info   (){ printf "$(tput setaf 3)[composer] %b$(tput sgr0)\n" "$1" >&2; }
print_success(){ printf "$(tput setaf 2)[composer] %b$(tput sgr0)\n" "$1" >&2; }
...
custom_bootstrap(){
  ...
  print_info "Setting up the database (rake db:create)"
  docker-compose run web bundle exec rake db:create
  handle_error $? "setting up the database"

  print_info "Running migrations for test environment (rake db:migrate APP_ENV=test)"
  docker-compose run web bundle exec rake db:migrate APP_ENV=test
  handle_error $? "setting up the database"
}
## /CUSTOMIZE
...
DB_DATA_CONTAINER_NAME=neofrogmi-services-db-data
SERVICE_PREFIX=$(echo ${PWD##*/} | tr -d '_')
...
start(){
  ...
  command -v docker-machine > /dev/null 2>&1 && URL=http://$(docker-machine ip default 2> /dev/null):$PORT/
...
clean(){
  stop_container_by_name $SERVICE_PREFIX
  remove_container_by_name $SERVICE_PREFIX
  remove_image_by_name $SERVICE_PREFIX
}
...
case "$1" in
  ...
  "console")
    print_info "Running"
    print_info "  docker-compose run web bundle console"
    docker-compose run web bundle console
  ;;
